...
with app.app_context():
    ...
    print("[SEED] Criando professores...")
    ...
    print("[SEED] Criando oficinas...")
    of_hist = Oficina(nome="História do Brasil", descricao="Oficina de introdução à história brasileira", vagas=30, ano_mes="2025-05", professor=prof_ana)
    of_mat = Oficina(nome="Matemática Divertida", descricao="Jogos e dinâmicas com matemática básica", vagas=25, ano_mes="2025-05", professor=prof_joao)
    of_teatro = Oficina(nome="Teatro Infantil", descricao="Expressão corporal e teatro para crianças", vagas=20, ano_mes="2025-05", professor=prof_carla)
    db.session.add_all([of_hist, of_mat, of_teatro])
    db.session.commit()

    print("[SEED] Criando turmas...")
    ...
    print("[SEED] Matriculando alunos nas turmas...")
    turma_mat.alunos.extend([aluno2, aluno3])
    turma_tea.alunos.append(aluno4)
    db.session.commit()

    print("[SEED] Registrando presenças...")
